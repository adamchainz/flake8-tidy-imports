...
syntax is the same, such as ``decimal.Decimal``. There is also a special directive
to ban a preselected list of removed/moved modules between Python 2 and Python 3. It
can be turned on by adding ``{python2to3}`` to the list of ``banned-modules``.


Whilst it can be passed on the commandline, it's much easier to configure it in your
config file, such as ``setup.cfg`` - for example:
